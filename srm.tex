...
We therefore wish to test the null hypothesis $\theta = \btheta_0$ vs.\ $\theta = \triangle^d \setminus \btheta_0$.
...
\begin{align}
    x_i | \theta, M_1 &\sim \text{Multinomial}(1,\theta), \hspace{1cm} \text{independently for }\, i=1,2,\dots\\
  \theta | M_1 &\sim \text{Dirichlet}(\balpha_0).\notag
\end{align}
...
\end{equation}
where $\alpha_t = \alpha_{t-1}+x_t$ and $O_0(\btheta_0)=p(M_1)/p(M_0)$ (see appendix section \ref{app:posterior_odds}).
...
\begin{thm}  
  ...
  Let $x_t\sim \mathrm{Multinomial}(1,\theta)$ for all $t in \mathbb{N}$.
...
\begin{equation}
  ...
  \mathbb{P}_{\theta = \btheta_0}\left( \exists t \in \mathbb{N}: O_t(\btheta_0) \geq 1/u \right) \leq u,
\end{equation}
...
\begin{equation}
  ...
  \mathbb{P}_{\theta = \btheta_0}\left( \exists t \in \mathbb{N}: p_t \leq u \right) \leq u.
\end{equation}
...
\todo{maybe make this a corollary}
...
\begin{equation}
  ...
  \frac{1}{t} \log O_t(\btheta_0) \rightarrow D_{KL}(p(\cdot|\theta_{\star}) || p(\cdot|\btheta_0)) \hspace{1cm} a.s.
\end{equation}
as $t \rightarrow \infty$, for all choices of $\balpha_0$, where $D_{KL}(p(\cdot|\theta_{\star}) || p(\cdot|\btheta_0))$ is the Kullback Leibler divergence of a multinomial distribution index by parameter $\btheta_0$ from a multinomial distribution indexed by parameter $\theta_{\star}$.

\end{thm}
\noindent When $\theta_{\star} \neq \btheta_0$, we must have $D_{KL}(p(\cdot|\theta_{\star}) || p(\cdot|\btheta_0)) > 0$, and so Theorem~\ref{thm:consistency} can be restated as $O_t(\btheta_0) \rightarrow \infty$ almost surely as $t \rightarrow \infty$, which consequently rejects the null almost surely for any choice of Type-I error probability $u$.
...
\begin{align*}
 \frac{1}{t} \log BF_{10}(\bx_{1:t}) = - \frac{1}{t}\sum_{i=1}^{t}\log \left( \frac{p(x_{i}|M_\star)}{p(x_{i}|x_{1:i-1},M_1)}\right) + \frac{1}{t}\sum_{i=1}^{t} \log  \left( \frac{p(x_{i}|M_\star)}{p(x_{i}|M_0)}\right),\\
\end{align*}
...
To adress the first term, note that as $t$ becomes large the Dirichlet posterior $p(\theta|\bx_{1:t}, M_1)$ concentrates mass on $\theta_\star$, consequently bringing the posterior predictive $p(x_{t+1}|\bx_{1:t},M_1) =\int p(x_{t+1}|\theta,M_1),p(\theta|\bx_{1:t},M_1) d\theta$ closer to the ``true'' density $p(x_{t+1}|M_{\star})$.
...
\mathbb{P}_{\theta_{\star}}\left( O_n(\btheta_0) \geq 1/u, \, \forall n \geq N\right) \geq 1-\delta.
\end{equation}
...
\[
  ...
  \left\{
    ...
    \leq
    \frac{1}{2}\Vert \btheta_0- \theta_{\star}\Vert_{1}^2
  \right\},
\] for any choice of $\rho > 0$. \todo{Minimize bound with respect to rho?}
\end{thm}
...
Consider null and true parameter value of $\btheta_0=[\frac{1}{2}\, \frac{1}{4}\, \frac{1}{4}]$ and $\theta_\star=[\frac{1}{2}\, \frac{1}{8}\, \frac{3}{8}]$ respectively.
The Kullback Leibler divergence of the null from the true multinomial distribution is $D_{KL}(p(\cdot|\theta_\star)||p(\cdot||\btheta_0)) = 0.0654$ (to 4dp).
A simulation of 1 million data points with $x_i \sim \text{Multinomial}(1, \theta_\star)$ and a Dirichlet prior with $\balpha_0 = 100\btheta_0$ was used.
...
The blue line shows $\log(BF_{10}(\bx_{1:t}))/t$ and the red dashed line shows $D_{KL}(p(\cdot|\theta_\star)||p(\cdot||\btheta_0)) = 0.0654$.}
    \label{fig:lbf}
  \end{figure}
...
Each blue trace is an independent simulation of $\hat{\rho}(\bx_{1:t})$ up until $t=1000$ or the first $t$ for which it crosses the rejection boundary of the Chi-squared test,visualized with a red dot, whichever comes sooner.
  ...
  \end{figure}
  \noindent To add some intuition to figure \ref{fig:ssrm_reject}, it can be shown that the rection region at time $t$ converges to $[0,1]\setminus \btheta_0$, yet $\btheta_0 \rightarrow \theta_\star$ $a.s.$ as $t\rightarrow \infty$.
...
\end{equation}
This, with $O_0(\btheta_0)=1$, implies that $I_t(u) = \lbrace \theta \in \mathbb{S}^d : O_t(\theta) \leq 1/u \rbrace = \lbrace \theta  \in \mathbb{S}^d : p(\theta| M_1)\leq p(\theta|\bx_{1:t}, M_1)/u \rbrace $, which is identified as the Bayesian \textit{support interval} proposed by \cite{support_interval}.
...
\begin{align}
  ...
  \frac{p(M_1|\bx_{1:t})}{p(M_0|\bx_{1:t})}  &= \frac{\int p(\bx_{1:t}|\theta,M_1)p(\theta,M_1)d\theta}{p(\bx_{1:t}|M_0)}\frac{P(M_1)}{P(M_0)},\\
                      ...
                      &=\frac{p(x_t|\bx_{1:t-1},M_1)}{p(x_t|\bx_{1:t-1},M_0)} \frac{p(M_1|\bx_{1:t-1})}{p(M_0|\bx_{1:t-1})},\\
    &=\frac{\int p(x_t|\theta,\bx_{1:t-1},M_1)p(\theta|\bx_{1:t-1},M_1)d\theta}{p(x_t|\bx_{1:t-1},M_0)}  \frac{p(M_1|\bx_{1:t-1})}{p(M_0|\bx_{1:t-1})} ,
\end{align}
...
The posterior distribution of $\theta| \bx_{1:t}, M_1 \sim \text{Dirichlet}(\alpha_t)$ where $\alpha_t = \alpha_{t-1}+x_t$ with $\balpha_0$ the initial prior parameter choice.
The posterior predictive densities are easily computed as
\begin{equation}
  \label{eq:posterior_predictive_m1}
   p(x_t|\bx_{1:t-1},M_1) = \frac{ \Gamma(\sum_i x_{t,i}+ 1)}{\prod_i \Gamma(x_{t,i} + 1)} \frac{\Gamma(\sum_i \alpha_{t-1,i})}{\prod_i \Gamma(\alpha_{t-1,i})} \frac{\prod_i \Gamma(\alpha_{t-1,i} + x_{t,i})}{\Gamma(\sum_i \alpha_{t-1,i} + x_{t,i})},
\end{equation}
and
\begin{equation}
  \label{eq:posterior_predictive_m2}
   p(x_t|\bx_{1:t-1},M_0) = \frac{ \Gamma(\sum_i x_{t,i} + 1)}{\prod_i \Gamma(x_{t,i} + 1)} \prod \theta_{0,i}^{x_{t,i}}.
 \end{equation}
...
\begin{lemma}(Martingale property of posterior odds under the null hypothesis)
  ...
  \noindent Let $x_i,$ be a sequence of $\text{Multinomial}(n_i,\theta)$ random variables and consider the sequence of posterior odds $O_t(\btheta_0)$ defined as in equation \eqref{eq:update_rule} with $O_0(\btheta_0)=1$, then $O_t(\btheta_0)$ is a nonnegative martingale under $M_0$.
  \label{lem:posterior_odds_martingale}
    \end{lemma}
  \begin{proof}
  \begin{align*}
    E_{M_0}[O_{t+1}(\btheta_0)|\mathcal{F}_t]  &= \int \frac{p(x_{t+1}|\bx_{1:t},M_1)}{p(x_{t+1}|\bx_{1:t},M_0)} O_{t}(\btheta_0) p(x_{t+1}|\bx_{1:t},M_0) dx_{t+1}\\
    &=  O_{t}(\btheta_0) \int p(x_{t+1}|\bx_{1:t},M_1) dx_{t+1}\\
    &=  O_{t}(\btheta_0),
  \end{align*}
...
Moreover let $D_{KL}(p(\cdot|\theta_\star)||p(\cdot|\bx_{1:t}, M_j) )$ denote the KL divergence of the posterior predictive distribution under model $j$ at time $t$ from the true multinomial distribution.
Let $A(q) = \lbrace \theta \in \mathbb{S}^d : D_{KL}(p(\cdot|\theta_\star)||p(\cdot|\theta) ) < q \rbrace$
\begin{lemma}
  \label{lemma:walker}
  If $\int_{A(q)} p(\theta|M_j) d\theta > 0$ only for, and for all, $q > \delta_j$, and $\lim \inf_t D_{KL}(p(\cdot|\theta_\star)||p(\cdot|\bx_{1:t}, M_j) ) \geq \delta_j$ a.s.
then
  \begin{equation}
    \label{eq:bayes_factor_convergence}
    \frac{1}{t} \log B_{10}(\bx_{1:t}) \rightarrow \delta_0 - \delta_1,
  \end{equation}
  provided that $\sum_t \frac{1}{t^2} \left( \mathbb{V}[\log \frac{p(x_t|\bx_{1:t-1},M_j)}{p(x_t|M_\star )}]\right) < \infty$.
\end{lemma}
\begin{proof}
  ...
  \begin{align*}
    ...
    =& -\log \frac{p(x_t|M_\star)}{p(x_t|\bx_{1:t-1},M_j)} + D_{KL}(p(\cdot|\theta_\star)||p(\cdot|\bx_{1:t-1}, M_j) ) +\\
    &S_{jt-1},\\
  \end{align*}
  from which it follows that $\mathbb{E}[S_{jt}|\mathcal{F}_t] = S_{jt-1}$ with $\mathcal{F}_t = \sigma(x_1,\dots,x_{t-1})$.
From the assumption that  $\sum_t \frac{1}{t^2} \left( \mathbb{V}[\log \frac{p(x_t|\bx_{1:t-1},M_j)}{p(x_t|M_\star )}]\right) < \infty$, it follows that $S_{jt}/t \rightarrow 0$ a.s.
and consequentially
  \begin{align*}
    \frac{1}{t} \log \frac{p(\bx_{1:t}|M_j)}{p(\bx_{1:t}|M_\star)} + \frac{1}{t}\sum_{i=1}^t  D_{KL}(p(\cdot|\theta_\star)||p(\cdot|\bx_{1:t-1}, M_j) )\rightarrow 0.
  \end{align*}
  From the additional assumption of $\lim \inf_t D_{KL}(p(\cdot|\theta_\star)||p(\cdot|\bx_{1:t}, M_j) ) \geq \delta_j$ a.s.
, it follows that
  \begin{align*}
...
Hence KL divergence of the null $\btheta_0$ from the true $\theta_\star$ is the relevant $\delta_0$ for model $M_0$ in lemma \ref{lemma:walker}.
...
The next step in the argument is to derive a confidence sequence for $S_n$ under some alternative hypothesis. In fact, we only need to derive a lower bound on $\KL (\hat\theta_n||\btheta_0)$ in order to apply the results of the previous section.
...
\begin{thm}\label{thm:power_lower_bound}
  Assume that $S_n$ has a multinomial $\tilde\theta$ distribution. For every $\balpha$, $\delta$, $\alpha$, $\tilde\theta$, and $\btheta_0$, there exists an $N$ such that
  \begin{equation*}
    P\left(\forall n \geq N: \KL(\hat\theta_n \Vert \btheta_0) \geq \Gamma_n(\balpha)\right)
    \geq 1-\delta.
  \end{equation*}
\end{thm}
\begin{proof}
  ...
  \[
    ...
    \df
    \left\{
    \KL(\hat\theta_n \Vert \btheta_0) \geq \Gamma_n(\balpha)
  \right\}.
\]
...
\[
  ...
  \left\{
    \Vert \hat\theta_n - \btheta_0\Vert_1 \geq \sqrt{2\Gamma_n(\balpha)}
  \right\}
  \supseteq
    \left\{
      \Vert \tilde\theta - \btheta_0\Vert_1
      -
...
\[
  \Vert \btheta_0- \tilde\theta\Vert_{1}
    \geq
        \frac{2}{n}\sqrt{\left(\frac{n}{4} + \rho\right)\log\pfrac{n/4+\rho}{2^{-2d}\delta^2 \rho}}
...
\begin{equation*}
    P\left(\forall n>0 \leq N: \KL(\hat\theta_n \Vert \btheta_0) \geq \Gamma_n(\balpha)\right)
    \geq 1-\delta,
  \end{equation*}
...
\begin{lemma}
  ...
  \begin{equation*}
    N = \Omega\left(
      \frac{d}{\Vert \btheta_0- \tilde\theta\Vert_{1}^2}
...
\[
  ...
  \left\{
    ...
    \leq
    \frac{1}{2}\Vert \btheta_0- \tilde\theta\Vert_{1}^2
  \right\},
\]
