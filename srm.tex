...
\begin{align}
    \bx_i | \theta, M_1 &\sim \text{Multinomial}(1,\theta), \hspace{1cm} \text{independently for }\, i=1,2,\dots\\
...
To adress the first term, note that as $t$ becomes large the Dirichlet posterior $p(\theta|\bx_{1:t}, M_1)$ concentrates mass on $\btheta_\star$, consequently bringing the posterior predictive $p(x_{t+1}|\bx_{1:t},M_1) =\int p(x_{t+1}|\theta,M_1),p(\theta|\bx_{1:t},M_1) d\theta$ closer to the ``true'' density $p(x_{t+1}|M_{\star})$.
...
\begin{align}
  ...
  \frac{p(M_1|\bx_{1:t})}{p(M_0|\bx_{1:t})}  &= \frac{\int p(\bx_{1:t}|\theta,M_1)p(\theta,M_1)d\theta}{p(\bx_{1:t}|M_0)}\frac{P(M_1)}{P(M_0)},\\
    ...
    &=\frac{\int p(\bx_t|\theta,\bx_{1:t-1},M_1)p(\theta|\bx_{1:t-1},M_1)d\theta}{p(\bx_t|\bx_{1:t-1},M_0)}  \frac{p(M_1|\bx_{1:t-1})}{p(M_0|\bx_{1:t-1})} ,
\end{align}
...
The posterior distribution of $\theta| \bx_{1:t}, M_1 \sim \text{Dirichlet}(\balpha_t)$ where $\balpha_t = \balpha_{t-1}+\bx_t$ with $\balpha_0$ the initial prior parameter choice.
...
Let $A(q) = \lbrace \theta \in \mathbb{S}^d : D_{KL}(p(\cdot|\btheta_\star)||p(\cdot|\theta) ) < q \rbrace$
\begin{lemma}
  \label{lemma:walker}
  If $\int_{A(q)} p(\theta|M_j) d\theta > 0$ only for, and for all, $q > \delta_j$, and $\lim \inf_t D_{KL}(p(\cdot|\btheta_\star)||p(\cdot|\bx_{1:t}, M_j) ) \geq \delta_j$ a.s.
then
  \begin{equation}
