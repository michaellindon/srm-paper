...
Formally, let there be $d$ treamtent groups, $\theta_0 \in \triangle^d$ be a probability vector where element $\theta_{0,i}$ denotes the probability of any unit being assigned to treatment group $i$, and $x_j$ a random variable denoting the assignment outcome of the $j$th experimental unit.
...
We therefore wish to test the null hypothesis $\theta = \theta_0$ vs.\ $\theta = \triangle^d \setminus \theta_0$.
...
\end{align}
Prior mass is concentrated around $\theta_0$ by specifying $\alpha_{0,i} = k \theta_{0,i}$ for concentration parameter $k \in \mathbb{R}^+$, in line with a ``Jeffreys-type'' testing procedure - if the null were not at least somewhat plausible, then a statistical test would not be needed.
...
If $\alpha_0$ is integer valued, and noting that all but one of the $x_{t,j}$ is 1 with the others 0, then the recursive definition simplifies substantially to
\begin{align}
...
A pure Bayesian analysis could proceed by rejecting the null when $O_t(\theta_0) > c$ and reporting a posterior error probability of less than $1/(1+c)$ \citep[Chapter 5]{bernardo}.
...
Consider the sequence of posterior odds $O_t(\theta_0)$ defined as in equation \eqref{eq:update_rule} with $O_0(\theta_0)=1$, then
\begin{equation}
...
The uniform condition in equation \ref{eq:type_1_error} suggests to reject the null at time $\tau = \inf \lbrace t \in \mathbb{N}: O_t(\theta_0) \geq 1/u \rbrace$.
...
To prove that this test is not trivial, we prove that the test rejects the null when $\theta \neq \theta_0$ almost surely, or that the test is asymptotically power one in the words of \cite{robbins}.
...
\end{thm}
\noindent When $\theta_{\star} \neq \theta_0$, we must have $D_{KL}(p(\cdot|\theta_{\star}) || p(\cdot|\theta_0)) > 0$, and so Theorem~\ref{thm:consistency} can be restated as $O_t(\theta_0) \rightarrow \infty$ almost surely as $t \rightarrow \infty$, which consequently rejects the null almost surely for any choice of Type-I error probability $u$.
...
\noindent Let $x_i,$ be a sequence of $\text{Multinomial}(n_i,\theta_{\star})$ random variables with $\theta_{\star} \neq \theta_0$ and consider the sequence of posterior odds $O_t(\theta_0)$ defined as in equation \eqref{eq:update_rule} with $O_0(\theta_0)=1$ and a given choice of $\balpha_0$, then
for all $\delta > 0$ and $u > 0$ there exists $N > 0$ such that 
\begin{equation}
  \label{eq:finite_sample}
\mathbb{P}_{\theta_{\star}}\left( O_n(\theta_0) \geq 1/u, \, \forall n \geq N\right) \geq 1-\delta.
\end{equation}
...
\[
  ...
  \left\{
    ...
    \leq
    \frac{1}{2}\Vert \theta_0- \theta_{\star}\Vert_{1}^2
  \right\},
