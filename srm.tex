...
Such behavior is often observed when a practitioner believes there is an underlying issue in the implementation, and performs an unplanned $\chi^2$ test to investigate,
...
Furthermore, the posterior odds are equal to the Bayes factor under prior odds of unity, and so Theorem~\ref{thm:type_1_error} can also be established as a corollary of mixture sequential probability ratio test (mSPRT) of \cite{wald}.
...
To address the first term, note that as $n$ becomes large the Dirichlet posterior $p(\btheta|\bx_{1:n}, M_1)$ concentrates mass on $\btheta_\star$, consequently bringing the posterior predictive $p(\bx_{n+1}|\bx_{1:n},M_1) =\int p(\bx_{n+1}|\btheta,M_1),p(\btheta|\bx_{1:n},M_1) d\theta$ closer to the ``true'' density $p(\bx_{n+1}|M_{\star})$.
...
\begin{figure}[H]
  ...
  \caption{Rejection boundaries for the proposed test ($\balpha_0 = 100\btheta_0$) in magenta and the $\chi^2$ test in green for testing the null hypothesis $\btheta_0 = [\frac{1}{2} \, \frac{1}{2}]$ at a $u=0.05$ level.
}
    \label{fig:critical}
  \end{figure}
...
The orange and magenta curves are the same as in figure \ref{fig:critical}.
Each blue trace is an independent simulation of $\hat{\rho}(\bx_{1:n})$ up until $t=1000$ or the first $n$ for which it crosses the rejection boundary of the proposed test, visualized with a red dot, whichever comes sooner.
}
    \label{fig:ssrm_fp}
  \end{figure}
...
Each blue trace is an independent simulation of $\hat{\rho}(\bx_{1:n})$ up until $t=1000$ or the first $n$ for which it crosses the rejection boundary of the proposed test, visualized with a red dot, whichever comes sooner.
  ...
  \end{figure}
  \noindent To add some intuition to figure \ref{fig:ssrm_reject}, it can be shown that while the rejection region at time $n$ converges to $[0,1]\setminus \rho_0$, the MLE $\hat{\rho}(\bx_{1:n}) \rightarrow \rho_\star \neq \rho_0$ $a.s.$ as $n\rightarrow \infty$ by the strong law of large numbers, and is hence guaranteed to cross the rejection boundary.
...
While our application of detecting errors in simply randomized experiments focused on a specific Multinomial-Dirichlet test, the same mathematical techniques can be obviously used to generalize these results to other Bayesian tests.
...
The authors proposed this as an alternative to the more commonly reported Bayesian credible intervals, arguing that the support interval is based on evidence in the data (how the data changes belief), whereas credible intervals are based on posterior belief directly.
...
This result follows from the application of the following two lemmas.
...
\end{proof}
The result of the Theorem is obtained by using Lemma~\ref{lem:posterior_odds_martingale} to establish that the posterior odds $O_n(\btheta_0)$ are a nonnnegative supermartingale under the null hypothesis, and using this observation in Lemma~\ref{lem:durrett} together with  $O_0(\btheta_0) = 1$  to obtain the main inequality of the Theorem.
...
These ideas follow through to lower bounding the Beta function. The lower bound on the Beta function follows by applying the lower bound to the Gamma functions
